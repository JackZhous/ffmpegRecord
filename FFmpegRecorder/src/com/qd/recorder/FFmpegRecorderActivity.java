...
public class FFmpegRecorderActivity extends Activity implements OnClickListener, OnTouchListener {
	...
	private void initHandler(){
		mHandler = new Handler(){
			@Override
			public void dispatchMessage(Message msg) {				//dispatchMessage是分发消息
				switch (msg.what) {
				/*case 1:
					...
					break;*/
				case 2:																										//手第一次按住，松开以及切换场景去读取不同的文字
					int resId = 0;
				...
				case 3:
					if(!recording)
						initiateRecording(true);					//开始前录制的初始化工作，记录了暂停时间等
					else{
						//更新暂停的时间
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mWakeLock.acquire(); 			//获取锁来保持屏幕处于亮起状态
	...
	private void initVideoRecorder() {
		...
		RecorderParameters recorderParameters = Util.getRecorderParameter(currentResolution);					//获取当前视频音频等级的参数
		...
		fileVideoPath = new File(strVideoPath); 					//strVideoPath  是一个类似时间戳.mp4格式的文件
		...
		videoRecorder.setFormat(recorderParameters.getVideoOutputFormat());				//视频格式  mp4
		videoRecorder.setSampleRate(recorderParameters.getAudioSamplingRate());		//采样频率
		videoRecorder.setFrameRate(recorderParameters.getVideoFrameRate());				//视频流频率
		videoRecorder.setVideoCodec(recorderParameters.getVideoCodec());						//编码器
		videoRecorder.setVideoQuality(recorderParameters.getVideoQuality()); 					//视频质量
		videoRecorder.setAudioQuality(recorderParameters.getVideoQuality());					//音频质量
		videoRecorder.setAudioCodec(recorderParameters.getAudioCodec());						//音频解码器
		videoRecorder.setVideoBitrate(recorderParameters.getVideoBitrate());						//比特率
